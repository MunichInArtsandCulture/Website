...
const OPEN_CALLS_API_URL = "https://script.google.com/macros/s/AKfycbxt-kfypE4qyARXGDPL1M9ze756WZbyljwQq-eeeZ_3Fy7PyEsrc9CZTh7v7STUmNI/exec"
  ...
  return (
    <>
      ...
      <div className="filter" style={{ display: 'flex', gap: '6px', marginBottom: '2rem', flexWrap: 'wrap' }}>
        <button style={modeBtn(mode === 'jobs')} onClick={() => setMode('jobs')}>Jobs</button>
            ...
            <div className="filter mobile-only" style={{ marginBottom: '2rem' }}>
              <div className="custom-dropdown" ref={dropdownRef} style={{ zIndex: dropdownOpen ? 1001 : 999, width: '220px' }}>
                <button
                  ...
                  style={{ width: '100%', fontSize: '15px', border: '1px solid #b6d8cf', borderRadius: '20px', display: 'flex', alignItems: 'center', justifyContent: 'flex-start', gap: '8px', padding: '10px 15px', background: btnHover ? '#685769' : '#1e7a62', color: 'white', transition: 'background 0.2s ease' }}
                ...
                >
                  ...
                  <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{CATEGORY_LABELS[selectedCategory] || selectedCategory}</span>
                </button>
                <ul id="dropdown-options" className={dropdownOpen ? '' : 'hidden'} style={{ background: 'white', border: '1px solid rgb(182, 216, 207)', borderRadius: '20px', marginTop: '2px', padding: '0px', overflowY: 'auto', overflowX: 'hidden', maxHeight: '300px', WebkitOverflowScrolling: 'touch', width: '100%' }}>
                  {CATEGORY_LIST.map((cat, idx) => (
                    <li
                      ...
                      style={{ display: 'flex', alignItems: 'center', padding: '12px 15px', margin: '10px', borderBottom: idx === CATEGORY_LIST.length - 1 ? 'none' : '1px solid #b6d8cf', color: '#363636', fontSize: '15px', cursor: 'pointer' }}
                    >
                      {CATEGORY_LABELS[cat] || cat}
                    </li>
                  ))}
            ...
            <ul className="blog">
              {openCalls.length === 0 ? (
                ...
                openCalls.map((call, i) => (
                  <li key={i} style={{ paddingTop: '18px', paddingBottom: '18px' }}>
                    {/* Category / type pill */}
                    {(call.category || call.type || call.Category || call.Type) && (
                      <div style={{ marginBottom: '10px' }}>
                        <span style={{ display: 'inline-block', background: 'rgba(104, 87, 105, 0.88)', color: 'white', fontSize: '13px', fontWeight: 350, padding: '0px 11px', borderRadius: '20px', whiteSpace: 'nowrap', fontFamily: 'Inter, sans-serif' }}>
                          {call.category || call.type || call.Category || call.Type}
                        </span>
                      </div>
                    )}
                    {/* Title */}
                    <h3 style={{ color: '#252525', fontWeight: 700, margin: '0 0 8px 0', letterSpacing: '-0.5px', fontSize: '22px', lineHeight: '1.3', fontFamily: 'Inter, sans-serif' }}>
                      {call.title || call.Title || call.name || call.Name}
                    </h3>
                    {/* Organisation */}
                    {(call.organization || call.organisation || call.Organization || call.Organizer || call.organizer) && (
                      <div style={{ color: 'rgb(104, 104, 104)', fontWeight: 600, fontSize: '16px', fontFamily: 'Inter, sans-serif', marginBottom: '6px' }}>
                        {call.organization || call.organisation || call.Organization || call.Organizer || call.organizer}
                      </div>
                    )}
                    {/* Deadline */}
                    {(call.deadline || call.Deadline) && (
                      <div style={{ color: '#685769', fontSize: '14px', fontFamily: 'Inter, sans-serif', marginBottom: '10px', fontWeight: 500 }}>
                        Deadline: {call.deadline || call.Deadline}
                      </div>
                    )}
                    {/* Link */}
                    {(call.link || call.Link || call.url || call.URL) && (
                      <a
                        href={call.link || call.Link || call.url || call.URL}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="apply-now-btn"
                        style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', color: '#685769', fontWeight: 600, fontSize: '16px', fontFamily: 'Inter, sans-serif' }}
                      >
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 14 14">
                          <path fill="currentColor" fillRule="evenodd" d="M13.854.146a.5.5 0 0 1 .113.534l-5 13a.5.5 0 0 1-.922.027l-2.091-4.6L9.03 6.03a.75.75 0 0 0-1.06-1.06L4.893 8.046l-4.6-2.09a.5.5 0 0 1 .028-.923l13-5a.5.5 0 0 1 .533.113" clipRule="evenodd" />
                        </svg>
